...
export class SatelliteManager {
  ...
  /*
   Resolve satellite type based on name
   Used to color satellites differently
  */
  _resolveTypeKey(name = "", catalogGroup = "") {
    ...
    if (n.startsWith("flock") || n.startsWith("skysat"))
      return "planet";
    ...
    if (catalogGroup && catalogGroup !== "active")
      return catalogGroup;
  ...
  async addSatellite(norad) {
    ...
    return this.addSatelliteFromTle(
      { norad: noradText, ...tle },
      ""
    );
  }
  ...
  update() {
    ...
    this.satellites.forEach((sat) => {
      ...
      const satellitePoint = latLonToVector3(
        latitude,
        longitude,
        altitudeKm
      );

      const groundPoint = latLonToVector3(
        latitude,
        longitude,
        0
      );
  ...
  createConstantOrbitLine(sat, startTime) {
    ...
    for (let i = 0; i <= ORBIT_SAMPLE_POINTS; i++) {
      ...
      const time = new Date(
        startTime.getTime() + progress * orbitPeriodMs
      );
  ...
  updateGroundLine(sat, groundPoint) {
    const previousPoint =
      sat.groundTrackPoints[
        sat.groundTrackPoints.length - 1
      ];

    if (
      !previousPoint ||
      previousPoint.distanceTo(groundPoint) >=
        MIN_GROUND_STEP
    ) {
    ...
    sat.groundLine.geometry.setFromPoints(
      sat.groundTrackPoints
    );
  }
  ...
  replaceLine(
    ...
    if (segments) {
      sat[key] = new THREE.LineSegments(
        geometry,
        material
      );
    } else {
      ...
      if (closed) {
        sat[key].geometry.setFromPoints([
          ...points,
          points[0],
        ]);
      }
