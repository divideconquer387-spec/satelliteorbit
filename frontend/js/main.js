...
function renderSatPills() {
  ...
  if (satellites.satellites.length > MAX_VISIBLE_PILLS) {
    ...
    overflow.textContent =
      `Showing ${MAX_VISIBLE_PILLS} of ${satellites.satellites.length} satellites`;
...
function getPickedSatelliteFromEvent(event) {
  ...
  return satellites.satellites.find(
    (sat) => sat.norad === pickedNorad
  ) || null;
}
...
renderer.renderer.domElement.addEventListener("click", (event) => {
  ...
  selectSatellite(
    pickedSatellite,
    `Selected NORAD ${pickedSatellite.norad} from map.`
  );
});
